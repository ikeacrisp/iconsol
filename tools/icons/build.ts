...
function generateIconCountModule(icons: LoadedIcon[]): string {
  const count = icons.filter((x) => x.record.display).length;
  return `// AUTO-GENERATED by tools/icons/build.ts — DO NOT EDIT.
...
const ICON_COUNT_MODULE = path.join(
  WEB_PUBLIC,
  "..",
  "src",
  "lib",
...
function main() {
  ...
  if (writeIfChanged(ICON_COUNT_MODULE, generateIconCountModule(all))) {
    console.log("  wrote apps/web/src/lib/icon-count.ts");
  }
