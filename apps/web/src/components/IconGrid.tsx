...
function CategoryIcon({
  ...
  useEffect(() => {
    const enteringHover = state === "hover" && prevState.current !== "hover";
    prevState.current = state;
    if (enteringHover) {
      pathLength.set(0);
      const controls = animate(pathLength, 1, {
        duration: STROKE_TRACE_DURATION,
        ease: [...STROKE_TRACE_EASE],
      });
